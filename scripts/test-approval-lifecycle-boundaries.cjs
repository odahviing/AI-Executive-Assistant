...
for(const kind of ['reschedule','oof'])for(const nudged of [false,true])test('R4 '+kind+' checking '+(nudged?'after sole nudge preserves expiry':'first reply keeps one reask'),async()=>{
 ...
 put('db/jobs',{updateOutreachJob(){}});put('db',{appendToConversation(){}});put('db/people',{});put('connectors/graph/calendar',{});put('connectors/graph/calendarReads',{});put('utils/responseDeadline',{});put('utils/scheduleRules',{});put('utils/shadowNotify',{});put('utils/extractJson',{extractFirstJsonObject:x=>x});put('utils/logger',log);
