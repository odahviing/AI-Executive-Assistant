...
/** Release every active hold for this holder in this conversation thread — the
 *  re-pick case ("actually, slot 2"): a new pick in the SAME negotiation
 *  replaces the prior hold rather than stacking toward the cap. */
export function releaseHoldsForHolderThread(
  ownerUserId: string,
  holderSlackId: string,
  originThreadTs: string | null,
  reason: string,
): number {
  if (!originThreadTs) return 0;
  const db = getDb();
  const res = db.prepare(`
    UPDATE slot_holds
    SET status = 'released', closure_reason = ?, closed_at = datetime('now')
    WHERE owner_user_id = ? AND holder_slack_id = ? AND origin_thread_ts = ? AND status = 'active'
  `).run(reason, ownerUserId, holderSlackId, originThreadTs);
  return res.changes;
}
