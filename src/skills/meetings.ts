...
- Colleague-path \`create_meeting\` / \`move_meeting\` runs a server-side rule check (work hours, work days, lunch / floating-blocks, focus-time, busy collisions, category day_type / per_day / per_week limits). On refusal the tool result includes \`broken_rule_label\` — a short human phrase like "outside ${firstName}'s work hours" / "in his lunch window" / "over his per-day Interview limit". Paste that phrase VERBATIM into \`create_approval(kind=policy_exception).ask_text\` (or \`meeting_reschedule\` for moves). Example: "Maayan wants Wed 10:30. That's outside ${firstName}'s work hours — override and book, or pick a different time?"
