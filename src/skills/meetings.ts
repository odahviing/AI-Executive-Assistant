...
ALWAYS prefer \`candidate_slots\` over multiple separate calls when the candidates are concrete times the user named. ONE call instead of N saves real time. \`search_from\` / \`search_to\` are ignored in this mode — pass any value (the candidate range is used).`,
        input_schema: {
          ...
          properties: {
            ...
            attendee_emails: { type: 'array', items: { type: 'string' }, description: 'The RESOLVED emails of EVERY person named for this meeting — resolve names via find_slack_user FIRST (internal colleagues resolve from people_memory, returning email + timezone). A partial list (missing a named attendee) produces slots that ignore that person and then fail at booking. Never search with only some of the named people.' },
...
RESOLVE WHO BEFORE WHEN — resolve every named attendee BEFORE calling find_available_slots. A request that names people ("30 min with Idan and Lori") is a search for ALL of them. A bare first name with no external cue is INTERNAL → call find_slack_user (it checks people_memory): one unambiguous match → use their email (their timezone comes with it); multiple or none → ask the requester "who is X?" — NEVER search a partial list or book without a named person. NEVER ask a colleague for a teammate's INTERNAL email you can look up — only ask for an email when the person is clearly EXTERNAL (a candidate, another company, a personal domain, usually flagged by extra context). A slack_id you were given → email + timezone are derivable, no ask. Then attendee_emails is the FULL resolved set, not just whoever you already had.
