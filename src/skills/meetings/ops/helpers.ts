...
// Render the instant in the stated output frame, including UTC-tagged inputs.
export function formatIsoTime(iso: string, timezone: string): string {
...
// the original start AND end, independent of the new duration. ONE helper, called from BOTH move return
...
export function computeVacatedSlot(
  ...
  preMoveEndIso: string | undefined,
  timezone: string,
): { start: string; end: string; label: string } | undefined {
  if (!preMoveStartIso || !preMoveEndIso) return undefined;
  const vs = DateTime.fromISO(preMoveStartIso, { zone: timezone });
  const ve = DateTime.fromISO(preMoveEndIso, { zone: timezone });
  if (!vs.isValid || !ve.isValid || ve <= vs) return undefined;
  return {
...
export function buildOutOfHoursBusy(
  ...
  const { ownerWorkIntervalsBetween } = require('../../../utils/workHours') as
    typeof import('../../../utils/workHours');
  const from = rangeStart.startOf('day');
  const until = rangeEnd.startOf('day').plus({ days: 1 });
  // Work intervals include adjacent trip dates that overlap this HOME query
  // range. Complement their union, so get_free_busy and checkSlot agree on
  // travel afternoons after home midnight and on DST wall-clock hours.
  const working = ownerWorkIntervalsBetween(from, until, profile);
  let cursor = from;
  for (const interval of working) {
    if (interval.start > cursor) blocks.push({ start: cursor.toISO()!, end: interval.start.toISO()!, status: 'oof' });
    if (interval.end > cursor) cursor = interval.end;
  }
  if (cursor < until) blocks.push({ start: cursor.toISO()!, end: until.toISO()!, status: 'oof' });
  return blocks;
}
