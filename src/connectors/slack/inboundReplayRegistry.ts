...
export interface InboundReplayParams {
  ...
  postThreadTs: string;
  /** The original inbound surface, retained for recovery diagnostics. */
  source: 'dm' | 'assistant_panel' | 'mpim' | 'channel';
  /** Surface flags: channel candidates mention the bot; MPIM candidates are
   * judged by the same addressee gate as live traffic. */
  isMpim?: boolean;
  isChannel?: boolean;
