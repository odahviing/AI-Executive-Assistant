 ...
 * are told (R3). Owner ruling 2026-07-25: two. It lives here, once — the two
 ...
 * on — the precise pair of wrong outcomes R3 exists to prevent. Every transition
...
function timersForWaitingSide(
  ...
  if (side === 'colleague') {
    ...
    // Owner-facing deadlines respect his work hours (R4) — same helper pair the
...
async function resolveRequestInner(
  ...
  // 'amend' — R7's counter cap doesn't apply, nothing here is a new offer
  ...
  // instead" (R6: a book may resolve to a move — no tool is off the table)
...
export async function notifyRequesterOfDecision(
  ...
  if (amendWithheld.length > 0) {
    // R3 — a withheld counter key is never a SILENT omission. `resolve_approval`
 ...
 * payload that is open-ended BY DESIGN (R6: his resolution may differ wildly from
 * the ask; R7: open-ended in KIND). Every other shape returned '' — so Maayan's
