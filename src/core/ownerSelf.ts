 ...
 * machinery (engagement rank, subjects + topic-beats, note timeline,
 * profile dimensions) works on the owner exactly the same way it works on
 * colleagues.
 *
 ...
 *     owner's own row (NULL-safe `slack_id IS NULL OR slack_id != owner` —
 *     see the comment at that query in db/people.ts) so he doesn't see
 *     himself as a contact.
