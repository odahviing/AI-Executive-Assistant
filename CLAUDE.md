...
Maelle runs on the GCP VM `maelle-agent-vm` (zone `europe-west4-b`) under PM2 —
**not on this laptop**. The local `logs/` dir is **STALE** (frozen at the
cutover); never diagnose a live problem from it. There is no local Maelle process
