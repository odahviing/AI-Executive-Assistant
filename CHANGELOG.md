...
- **Recovery window is "since Maelle was last online" — no time cap.** A persisted **socket-alive watermark** (`data/socket-watermark.json`, stamped only on real inbound + socket-connect — never the bare process timer, so a zombie's watermark correctly freezes) is the window. Off two days → recover two days; off a week → a week (no 24h cap — owner direction). **One reply per person**, to their single LATEST unanswered message across all surfaces (top-level DM + panel threads gather candidates; the newest wins). **Safety:** with no watermark (first run on this build / lost file) `oldest = now` — recovery replays **nothing** pre-existing, so the fix can't belatedly blast colleagues with the backlog the old registry-blind catch-up never answered. "What's gone is gone." ([socketWatermark.ts](src/connectors/slack/socketWatermark.ts), [background.ts](src/core/background.ts)) — verified by a 15-scenario `/trace` (15/15; the watchdog fail-safe was a gap the trace caught and fixed).
