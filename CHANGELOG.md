...
- **Two colleagues' timezones were already corrupted in production** (America/New York instead of Asia/Jerusalem) by the exact write-path 4.8.0 closed — the fix stopped it happening again, but couldn't undo what had already happened. Found by tracing why real meeting offers were landing at 11pm Israel time; corrected directly. Review of the 4.8.0 fix itself also found it could introduce a smaller version of the same bug class it was closing (a same-day travel note could silently overwrite an owner-STATED one) — closed with a guard before it ever fired for real.
- **The honest-hedge fix from 4.8.0 only covered half the reachable surface.** It correctly stopped a colleague-facing "confirm it with him directly" line from reaching the owner on the email leg; review found the identical line could still reach him on his own Slack DM, a path 4.8.0 never checked. Both now address him directly, via one shared predicate instead of a hand-typed check per call site.
- **A proactive social aside could get stuck on one topic indefinitely.** The 4.7.5 fix only freed a topic category when it had zero live subjects left — but the pick loop returned as soon as it found any category with something to continue, so a category that was ALSO fully exhausted, sorting later, never actually got evaluated or freed as long as another category kept winning. Confirmed: one topic recurred for over a week, spanning before and after that fix shipped. See the full redesign below, which replaces the mechanism this bug lived in rather than just patching the loop.
- **The owner-said-done scanner could DM the owner about his own message, in the third person** — a related but distinct case from the one 4.8.0 closed: a request's requester field could still end up pointing at the owner himself via a different path (an approval raised from a room/MPIM minted his own id as the requester, and a room-booked meeting could link back to him the same way). Both guarded.
...
- Several colleagues' timezone and identity records corrected directly (a name mixup between two people sharing initials, two display-name fixes, one stale test fixture removed).

### Changed — how a coda picks its topic
- **Topic category selection is now probabilistic, not deterministic.** The old picker always continued whatever category scored highest — which is exactly the shape of bug that got stuck on one game for a week. Now: three equal-probability slots. With 3 active categories, one of the three, evenly. With fewer, each empty slot is an independent chance to try something brand new instead — 1 active category means a 2-in-3 chance of exploring, not a 1-in-3 chance of variety within a single topic.
- **A category that still has real standing but nothing left to continue now raises a new subject inside itself, instead of being abandoned.** The 4.8.1 fix above (loop reaches every dry category) is superseded by this: a category you've built real engagement in stays yours — you keep talking about movies, just a different movie — rather than freeing its slot for something unrelated.
- **A subject can now go stale on a calendar, not just from being ignored.** "Vacation in August" doesn't need to be raised and ignored twice to stop being current once September arrives — inherently time-bound subjects now carry an expiry date, set by the same pass that already reads what was actually discussed. A subject dies from either path — two unanswered raises, or its own date passing — into the same single dead state.
- **A subject you explicitly waved off could resurface later as if it were still live** — rejection only cleared a marker, and the next unrelated pass could flip status back to live from the count alone. Fixed at the source; a rejection now sticks.
- **Bringing up an old topic yourself now gets recognized instead of silently duplicated.** The reconciler only ever looked at currently-live subjects, so mentioning something Maelle had already filed as dead (or that had expired) created a second, disconnected record instead of reviving the one that already existed.
- A fresh "what's new" question inside a category no longer risks re-asking about something already being tracked there.
...
- The topic-picker redesign was dispatched with an explicit instruction to find more than what was asked for, the same way this wrap's own bug hunt worked — it returned 5 issues beyond the specified design, including the resurrection bug above.
...
### Changed
- **Channel replies now require a fresh mention every turn, with no exceptions.** A carve-out let Maelle keep answering un-mentioned channel thread replies once she'd already spoken in that thread — resolved in favor of the charter's original strict rule (matching how the catch-up/downtime path already behaved). MPIM behavior is unaffected; a group DM can still address her without a literal @mention when the addressee gate reads a message as clearly hers.
...
- **The email leg gained an honesty check it never had.** The `owner_fact` check (added 2026-08-14, after the email leg already existed) runs everywhere else but was never wired into email replies — undocumented, not deliberate. It now runs there too. Separately, a comment describing why the availability-floor check is skipped on email claimed its hard-block ledger is "empty by construction" — it isn't (a Slack-colleague turn can arm it, with a 45-minute window); the comment now states the real, narrow, accepted risk instead of a wrong one.
- **A stated timezone read out of an email could permanently corrupt a colleague's real one, with nothing to self-correct it.** The system already has a proper mechanism for "this person is somewhere else for a while" — a dated, auto-expiring override, used elsewhere for exactly this. The email-inbound timezone extractor never used it: any zone the email itself stated (not the owner's own words) was written straight into the person's permanent record, no expiry, able to silently overwrite a correct Slack-verified zone forever the moment one email misattributed a stated zone to the wrong participant. Now routed through the same auto-expiring override every other "temporarily elsewhere" fact already uses; only the owner's own stated correction still writes permanently, as before.
- **An honest-hedge fallback shipped colleague-facing wording directly to the owner.** When a slot-offer or fact-check catch's own rewrite couldn't be trusted either, the safe fallback line said "confirm it with him directly" — correct when Maelle is telling a colleague something about the owner, meaningless when the reply goes straight to the owner himself (the email leg). Now addresses the owner directly in that case, in all four supported languages.
- **The owner-said-done scanner could DM the owner about his own message, in the third person.** When a request's requester field ended up pointing at the owner himself (an upstream authority-misclassification, same class already guarded elsewhere), the requester-relay had no owner-self check and would notify him that he'd closed his own item — plus an unnecessary shadow copy and outreach-job row keyed on himself. Guarded at both remaining call sites.
...
- The owner's real first name was hardcoded into 4 shipped prompt/tool-description strings — a clone of this codebase for a different executive would have told that executive's colleagues "sent to Idan to decide." Now interpolated from the profile where the strings are live, or swapped for a generic placeholder in illustrative examples.
- A handful of stale `file:line` citations across code comments and lane charter docs, corrected on sight.

### Migration
- `people_memory` gained a `name_set_by` provenance column (mirroring the existing `name_he_set_by`), so a Slack sync can no longer silently overwrite a stated name correction once one exists. No correction flow writes at that authority yet — this closes the sync half of the gap ahead of the correction flow being built.

### Data
- Corrected several colleagues' timezone/name records directly in production, independent of this code (Simon Arazi, Simy Giami, Leor Eliashiv and Asaf Magazanic to their real zones; a name mixup between two colleagues sharing initials; two display-name fixes; one stale test fixture removed). Found while tracing the timezone bug above.

### Framework
- The remaining five lanes (Librarian, SlackMaster, Diplomat, Instructor, Handyman) each ran a full charter-compliance audit against current code — all eight lanes have now been audited. Several small provable fixes landed inline per lane; the product/design-shaped findings each lane surfaced (channel-mention posture, the name-provenance gap, the email-leg gate coverage gap, an owner-name-hardcoding cloneability issue) were all resolved by the owner in the same version — see Changed/Fixed/Migration above.
...
- Two pre-wrap adversarial verify passes (forced to Fable) ran over the accumulated diff — zero overturns across both; three small discoveries fixed directly (dead return-side plumbing left over from the approval-dispatch removal, two stale citations).
- `SESSION_STARTER.md` corrected: it claimed no run of the Manager's bug loop or the Cleaner sweep is ever cron-armed — both are now live, scheduled Windows Task Scheduler jobs, and the doc said otherwise. Also corrected a stale rule-tag registry count.
...
### Framework (other chats, bundled)
Cleaner's own `audit` verdict renamed to `needs-judgment` — it shared a spelling with an unrelated, pre-existing ledger verdict meaning "a findings-only pass ran." A new durable per-run history (`run-history.jsonl`) now records three progressive completion stamps for every Manager run, so an unattended run that dies partway names exactly where it got to instead of just "failed." A local nightly cron now triggers the Manager's bug loop headlessly, unattended, with a dollar-cap safety net; a weekly cron does the same for the cleaner sweep.

A new `/report` skill gives a cheap way to read the standing report and record a decline/defer/queue-for-later verdict without a full Manager session — a `build`/`convert` verdict still hands off to the real thing. The shared builder charter (`WORKSHOP.md`) was rewritten for size and clarity: 3,179 → 2,668 words (−16%), consolidated from 13 rules to 12 (a duplicate-of-W2 rule folded in, a duplicated "done" definition merged into one place), plus a genuine new owner-decision case added (stop before building a new tool/skill/spine, not just after). Diplomat and librarian's charters were measured and trimmed for the same reason — restated ideas, not missing scope; matchmaker was confirmed to already be the leanest of the eight builders and left alone bar one missing pointer.
...
- The morning news brief could claim a source mentioned the owner's own company when it never did (an invented "shout-out" traced back to a LinkedIn post that never named it). Whether a search result actually mentions the tenant's own configured company is now a deterministic check before a source can be framed that way — generalized per tenant, never a hardcoded company name.

### Framework (other chats, bundled)
The nightly report table redesigned from five columns to four, moving file:line citations and mechanism jargon out of the owner's daily view into the ledger row alone. The Manager's `pendingOverflow` queue no longer silently loses a discovery when a wrap runs mid-drain, and a backfill script's "confirmed against production" claim can no longer mean a stale local database copy. A concurrent-architect-session collision risk (two sessions editing the same charter row) gets a documented pre-write check. Stale-citation detection no longer flags a symbol cited at its use site, only a truly drifted one; the `P` rule-tag is now fully retired from source comments. Cleaner's own `audit` verdict is renamed to `needs-judgment` — it shared a spelling with an unrelated, pre-existing ledger verdict meaning "a findings-only pass ran," which the mapping table had to work around instead of the collision being removed at the source.
...
- An approval could get executed (a meeting actually booked) but never formally close, when a failed automatic replay led to a direct booking outside the normal link — leaving it to eventually produce a false "still waiting" reminder to the owner and a false "no read" to the requester. A new closure check now recognizes this case retroactively.
...
### Also in this version — the decisions behind the parked findings
The owner ruled on the five parked findings and four longer-standing questions rather than carrying them. Three were declined outright, two closed at zero cost once re-checked, and the rest built. Two of his rulings **reversed an earlier decline** after the item was written up properly — in both cases the fuller version contained something the one-line summary had hidden.

- **The autofix no longer redoes what he undid.** Only the explicit revert *tool* recorded a decision; a conversational *"move it back"* wrote nothing, and the sole protection was a 12-hour window timed from **Maelle's** move rather than **his** — so undoing something 13 hours later let the next sweep redo it. An owner-authenticated move of a recently auto-moved event now records the dismissal itself, expiring after one full detection cycle. Fixing it exposed two adjacent faults: the dismissal's row lookup was keyed on the event alone, so a day-shape dismissal would have silenced an open *"which category?"* question — this version's own memory fix, in reverse — and an expired terminal row had been silencing **tracking** of an event forever while detection had resumed.
- **A colleague is told when a promise to him is voided.** Previously a later write closed his reschedule ask as a success while the time he had been given no longer existed, and nobody told him. He is now corrected — but **only when the new time genuinely differs** from what he was told, capped once per event per day, and relayed through the same path that sent the original notice so it carries one lifecycle row, one expiry and one close-loop. The comparison is on instants rather than text, because the two values are formatted independently and equal moments differ as strings — a text compare would have made pestering the default.
- **A date-only free/busy window now means that whole day.** Asking about "Thursday" with no time had been answered from the blocks covering midnight and read as *"they're free."* Fixed in the connector so one rule serves every caller, which also closed a silent truncation where a date-only **range** read only its first day.
- **The third work-item lifecycle is gone.** Every colleague message had been minting a third record beside the request and its payload — with its own timer and no dispatcher, so it was marked `failed` at the deadline, silently, because the unknown-type path returns instead of raising. The type no longer admits it, so minting one is a compile error rather than a convention. A user-visible bug fell out: the completion tick had been racing ahead of the send, so **a failed send still got a ✅**.
- **A migration stopped writing a backup file on every boot** for a duplicate-identity pair that can never be merged. The gate is exact rather than heuristic — a unique index on the identity column means one nullable row in a group is precisely the condition under which a merge exists to do.
- Two items **closed at no cost** once re-checked against the code: a reserved relaxed-rule seam that this version's authorization work had already made unreachable, and a narration flag whose cited lines no longer existed — where the "fix" would have filed a truthful in-hours conflict under a noise label and **erased the day's real reason.**

### Not changed
- Declined after review, each with the reasoning recorded: a file-side deferral that reports as an identity refusal (a filesystem fault only, failing in the recoverable direction) · an unlinked reschedule row that no longer suppresses an autofix (needs two faults to reach a person) · non-outage Graph faults reaching her context as raw text (context hygiene; both callers are crash-guarded) · seven owner-facing relays posting to a synthetic thread id (code-provable but never observed, and the log that would show it has rotated) · an unmeasured tripling of scan work on a long search — explicitly not to be measured either.
- **A content-level "don't tell a colleague the same thing twice" rule is deliberately unbuilt.** The correction cap says *don't correct twice*, which is a different question, and answering the second one needs its own design rather than an extension of the first.
