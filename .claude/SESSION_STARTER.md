...
- **Report row ids are a source letter plus one flat number** — `G` github · `L` logs · `B` both · `O` owner, e.g. `G176`. One counter, so the number alone is unique and `gh#176` can never be mistaken for it. Defined once, in the Manager skill's report spec.
