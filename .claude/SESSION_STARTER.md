...
## Next up — live-test feedback on 3.7.3, then whatever surfaces

3.7.3 is shipped + pushed (all of 3.7.2's bug wave, **#143 work-time overrides**, and the **full-day WE-spine deletion**). Owner is deploying (`npm run deploy`) and running live tests — expect the next session to open on **real-day feedback from those tests**. The fresh surfaces to watch: work-time overrides ("Boston 9-5 EST" / "Monday night" / "off Tuesday"), the `revert_last_auto_move` tool, requester-aware colleague move/cancel, and **timezone correctness on away days** (the thing the owner cares about most). Standing mode unchanged: propose-first, code-first, root-cause; trace `logs/maelle-YYYY-MM-DD.log` + code before attributing. A deeper **`audit`-skill** internal bug hunt (rule engines / requests-spine / gate stack / boot-recovery / cross-TZ) remains a good option when there's no real-day queue.
