...
Off a long "book my whole week in Boston" real-day thread. The root of most of it: the booking **write-path had no travel context**. New keystone — **`resolveOwnerTravelContextForDate(date) → {isAway, effectiveTz, location}`** ([workingElsewhere.ts](src/utils/workingElsewhere.ts)), built from the all-day WE marker (+ travel-record fallback), wired into **create / move / update_meeting**: bare trip-time → interpreted in the trip TZ (bare "10am" in a Boston week = 10am Boston), location → trip place (not home-day Huddle/blank), confirmations display in trip time. **WE invariant preserved**: no marker → `isAway=false` → home TZ + empty location → every consumer is a no-op, in-office flow byte-identical. Deterministic TZ math lives in ONE shared util (`timezoneConvert.ts`, used by find/create/move); new `start_timezone` arg on create/move (explicit override; tool does the arithmetic, Sonnet only tags the zone). **F1** owner-path event-id ledger (`threadEventLedger.ts`) — edit in-thread events by id, never re-find by name. **F2** active-planning-window anchor (also from the ledger, **travel-free**) — bare day refs ("Thursday") resolve to the session's week, so "just plan my July" works with no trip. **2.2 close-loop**: a freeform approval no longer dead-ends — it stays `in_flight` until the booking/cancel lands + reconnects (grace timer `approval_action_timeout`). Plus: "Booked by Maelle" always leads the invite again; `find_available_slots` excludes already-offered slots so "another option" returns a new day (offered stash now accumulates the union); floating-block move preserves its own duration; truncated-event-id reuse ("The Id is invalid") killed. **Parallel chats folded in**: guard chat (claim-checker reasoning-leak fix, set_event_category OK marker), prompt/tenancy chat (category→set_event_category routing, one-language composition #3, suggestion-completeness #129).

**Restart (`npm run deploy`)** — new utils + in-memory ledgers; no schema change. **Known residuals:** first bare-date ref in a thread (empty ledger) may still need a clarify; G1 "stale proposal after a time-shift" (free-check not auto-re-run when proposed times change — booking check still catches it) is a prompt-chat note. **Watch live:** the travel-context on the real Boston week (bare times → Boston, location → Boston, "Thursday" → the trip week), and that the in-office flow is unchanged.
