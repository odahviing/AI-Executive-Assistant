...
## Where we are — v2.1.6 just shipped
...
v2.1.6 adds a centralized meeting-mutation cascade. New helper `src/utils/closeMeetingArtifacts.ts` is called after every `move_meeting` / `update_meeting` / `delete_meeting` — resolves pending approvals whose payload references the meeting_id to `superseded`, closes sibling `outreach_jobs` with `intent='meeting_reschedule'` referencing the same meeting, cancels `follow_up` / `reminder` tasks linked by meeting_id in payload, and cancels their sibling expiry/reminder/decision follow-up tasks. Fixes "Yael asked to move, we handled it, but 'Needs your input' + active reminder still show in open tasks." Also: `getCalendarEvents` now paginates via `@odata.nextLink` (was `$top: 100` single-shot, silently truncated multi-week queries); `delete_meeting` verifies post-delete via `verifyEventDeleted` helper (GET returns 404) and narrates honestly when Graph returned 200 but the event is still there; `scrubInternalLeakage` strips `AAMk...` Graph event IDs so raw identifiers can't leak into owner-facing text.
