...
Working on Maelle at `E:/Code/Maelle`. **Current version: v4.3.5** — `package.json` is the source of truth. **HEAD = the 4.3.5 wrap** (run `git log -1` for the SHA). 4.3.5's whole shape is worth knowing before you touch a prompt: **the only executable change in it is one ternary**, and four of its six fixes were the same defect — *the text Maelle is given contradicted the code it describes*. Five of six were verified by the pre-wrap pass; the one it overturned was corrected before release, and three text corrections shipped on targeted re-reads rather than a second full pass (the CHANGELOG names them). **Prompt growth is now measured per tier** — that wave was ~+1,750 chars, and the always-on tier ended smaller than it started. Version-by-version history is in `CHANGELOG.md`, which is canonical — this file carries only what a session needs before it acts.
...
- **Framework findings live on `X`, the architect's charter rules on `A`** — `X1`–`X76` in `.claude/agent-loop/architect-ledger.jsonl`, `A1`–`A12` in `.claude/agents/architect.md`. Renamed off `A` on 2026-07-31 so one letter never means two things. **Before filing, check whether the row already exists** — a repeat is resent with an update, never given a new number, and `architect-file.cjs` refuses one it recognises. **A new row takes the lowest FREE number, not the next one**: merged duplicates release their ids and those gaps get filled, which is how the ledger stays around a hundred live rows. A number is only free once a grep for it returns zero everywhere.
- **There is a ceiling on how many items one run may put on his desk** (12, measured from seven runs). It warns on a run's own findings and it **gates** what the backlog may add to the report.
- **The backlog is a source on EVERY run** (his ruling: *"put backlog in every run"*), and its top reaches the report — bounded by that ceiling, in harm order, `confirmed` rows only. `ledger-stats --open` splits into *confirmed* · *need a re-read* · *cite no file*: **`confirmed` means a lane opened the code and stood behind the row**, and a row is re-read when its cited code MOVED **or nobody has ever looked at it** (28 of 52 on 2026-07-30, against 0 that had moved). A row citing no file can never be checked mechanically — those are named for a hand read. A re-read marks and confirms; it never closes on its own and never spends the decision budget.
...
**The live list is `node scripts/ledger-stats.cjs --open` (52 rows, 2026-07-30 — 11 confirmed, 28 never examined, 13 citing no file) and `gh issue list`.** Only what neither of those can tell you is kept here:
...
- **NEVER run the loop in a git worktree — it fails SILENTLY and looks like success.** `logs/` is gitignored (`.gitignore:25`), so a worktree has no log files at all: the scout's activity check counts zero `Orchestrator invoked` events, correctly returns `{issues: []}`, and reports a clean zero-finding log review while the entire window went unread. And every write to `report.md` / `state.json` / `agent-loop/` lands in the worktree, never reaching master, so the wrap sees none of it. Run from `E:/Code/Maelle` itself. This is why the spawned-task chip (which offers a fresh worktree) is the wrong door for a run — open a normal new chat instead.
