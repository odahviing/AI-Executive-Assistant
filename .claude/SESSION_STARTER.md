...
**v2.9.2 stabilizers layered on top** (note: the completeness gate that originally shipped in v2.9.2 was removed in the next patch — it was forcing Sonnet into a retry loop on inherently-incomplete asks and producing false "I sent it to Idan" claims when the requester legitimately hadn't shared a fact. `create_approval` now trusts Sonnet the same way `create_meeting` does: ask the requester conversationally if a fact is missing, or escalate honestly. No gate):
