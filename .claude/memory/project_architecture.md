...
---
Architecture reference for Maelle **v2.6.2** (channel thread-continuation + social skill rename + emoji feedback loop; v2.6.1 = recent-outbound context for colleague DMs + exact-slot rule check w/ broken_rule_label + claim-checker specifics-mismatch shield bypass; v2.5.0 baseline = per-thread queue + per-turn cache + email-first booking schema; v2.4.x = floating-block + preferences catalog; v2.0.0 = Connection interface milestone).

**Channel thread-continuation (v2.6.2).** Real-channel `message` event handler in `connectors/slack/app.ts` lets thread replies through when (a) the message is a thread reply (`event.thread_ts && event.thread_ts !== event.ts`) AND (b) Maelle has at least one assistant turn in that thread's `getConversationHistory(threadTs)`. Top-level channel chatter and threads she's never engaged with still drop. Once she's @-mentioned via `app_mention` and replies once, follow-up messages in that thread flow through the same MPIM relevance + addressee gates downstream — no repeated `@mention` needed. Skipped for channel continuations: the `<<GROUP DM>>` participant preamble (wrong framing for channels) and the full-channel members fetch (could be hundreds of members). Thread participants come through processMessage's existing `conversations.replies` merge.

**Social skill rename + single master toggle (v2.6.2).** `PersonaSkill` → `SocialSkill` (file `src/skills/persona.ts` → `src/skills/social.ts`); `skills.persona` → `skills.social` in profile YAML; variable `personaActive` → `socialActive` at all 5 gate sites (orchestrator pre-pass, social context block, coda gate, systemPrompt people-memory format, socialOutreachTick + socialDecay dispatchers). `loadUserProfile` projects legacy `persona: true` → `social: true` at parse time AND `getActiveSkills` migrates at registry load — defense-in-depth so old yamls boot without edits. `SkillId` union: `'social'` canonical, `'persona'` moved to legacy aliases. Master on/off lives only on `skills.social` — the redundant `behavior.proactive_colleague_social.enabled` field was retired (kept optional in schema for old-yaml boot, value ignored). Window/cooldown/skip-weekends sub-options under `behavior.proactive_colleague_social` unchanged.

**Proactive social recency loosening (v2.6.2).** `socialOutreachTick.ts`: fixed 72h recency gate replaced with TZ-aware local-work-week boundary. New `computeLocalWeekStartMs(zone, nowUtc)` returns Sunday 00:00 local for `Asia/Jerusalem`, Monday 00:00 local for everything else (default ISO week start). New `lastTopicTouchMs(personSlackId, ownerUserId)` queries `social_topics_v2.last_touched_at` for the most recent topic-touch on this person. Recency signal now `max(last_inbound, last_topic_touch)` so colleagues Maelle has logged topics on (engage-mode in conversation) qualify as "active" even without inbound DMs. Reject reasons updated: `silent_>72h` → `silent_this_week`, `never_inbound` → `no_signal_ever`. Pre-fix the tick fired 327× in 14 days picking zero candidates (gate too tight); post-fix, IL-TZ colleagues with topic activity this week qualify within their local 13:00-15:00 window.

**Emoji feedback loop (v2.6.2).** Four pieces wired through Slack reaction events + reply pipeline:
- **Approvals over emoji** — Slack `reaction_added` handler in `app.ts` matches `event.item.ts` against `approvals.slack_msg_ts` (new helper `getPendingApprovalByMsgTs` in `db/approvals.ts`), routes ✅/👍/🙏 → `resolveApproval(approve)`, ❌/👎 → `resolveApproval(reject)`, other emojis ignored. Owner-only via `event.user === approval.owner_user_id` check.
- **Colleague ack shadow loop (closes #56)** — when D4's `closeFollowupForMessageTs` matches an outreach_jobs row, ALSO fire a shadow DM to owner with WHO + emoji + message preview, threaded under the same `conversationKey` so it nests with the original outbound's shadow line.
- **`✅` on Maelle's reply at activity completion** — in `connectors/slack/postReply.ts:sendReply`, capture the `say()` response ts (Bolt returns `ChatPostMessageResponse` at runtime even though typed as `Promise<unknown>`) and react `:white_check_mark:` on Maelle's own message. Doesn't replace the user-message 🧵/👀 marker (kept untouched per owner direction).
- **Ack-class emoji replacement** — new `isPureAckReply(text)` helper detects pure short-ack phrases ("Got it", "On it", "Done", "Noted", "Sure", + variants, case-insensitive, ≤30 chars). When matched, `postOrchestratorReply` reacts `:+1:` on the user's message via the new `userMessageTs` field on `PostReplyInput` and skips the text post entirely. Conservative — content replies always post as text.

**Recent-outbound context for colleague DMs (v2.6.1, D4).** New `src/connectors/slack/recentOutboundContext.ts`. Solves the "Hey, what can I help you with?" amnesia where a colleague's reply to Maelle's proactive DM (informational, `await_reply: false`) landed without conversation context. New `outreach_jobs.followup_closed_at` + `followup_close_reason` columns track conversational closure independent of the existing `status` field (which churns at 5 min via closeFireAndForgetOutreach for await_reply=false rows). At inbound colleague-DM time the Slack connector calls `getRecentOutboundContext` BEFORE the orchestrator runs: ≤10min from outbound `sent_at` is a deterministic match (attach context, close as `deterministic_match`); 10min-24h is ambiguous and runs a small Sonnet classifier (RESPONSE → attach + close as `llm_response_match`; NEW_TOPIC → leave open, no context); >24h auto-expires lazily. Plus two explicit signals: a Slack `reaction_added` event handler closes followup as `emoji_ack` on emoji reactions to outbound DMs; thread replies (`inbound thread_ts === outreach_jobs.dm_message_ts`) close as `thread_reply` and use `buildThreadReplyContextBlock` for explicit framing. Existing pipelines (handleOutreachReply, meetingReschedule, outreachExpiry) auto-close `followup_closed_at` via `updateOutreachJob` when status flips to `replied`/`done`/`cancelled`/`expired`/`failed` (idempotent COALESCE) — no call-site changes. New `OrchestratorInput.priorOutboundContext?: string` carries the rendered prompt block; pinned at the top of the dynamic system prompt block so Sonnet sees it before drafting.

**Exact-slot rule check via diagnosticsOut (v2.6.1, B1).** `findAvailableSlots` accepts an optional `diagnosticsOut?: { rejectedCounts?, rejectedExamples? }` populated by reference at the end of the search — same data the existing `rejection_breakdown` log emits, now also queryable by callers. Used by colleague-path `create_meeting` / `move_meeting` Guard B at `skills/meetings/ops.ts` to pull the SPECIFIC rule that rejected a narrow-window slot, map to a one-phrase human label (`outside ${owner}'s work hours` / `in his lunch window` / `over his per-day Interview limit` / etc.), and surface as `broken_rule_label` on the refusal. Sonnet pastes verbatim into `create_approval(kind=policy_exception).ask_text`. Pre-v2.6.1 Guard B widened the search by ±60s "for tolerance" — but the slot loop strides 15-min from searchFrom, so cursor landed at start-1min and the requested slot was never tested (an office-day request at the exact `hours_start` boundary failed `outside_owner_work_hours`). Fix passes the exact (start, end) window. The widening defended against nothing concrete.

**Claim-checker specifics-mismatch shield bypass (v2.6.1, B2).** `ClaimCheckResult.claim_specifics_mismatch?: boolean` new field. The LLM judge prompt instructs it to set the flag when the over-claim names a SPECIFIC change the tool that ran doesn't cover (e.g. "updated to 25 min" while only `move_meeting` ran — start changed, duration didn't). The safety-net shield in `connectors/slack/postReply.ts` was previously coarse: any of the 6 mutation tools satisfied any `book`-class claim, so `move_meeting` wrongly masked a "duration changed" claim. Shield now bypasses retry suppression when the flag is true; the retry's `priorActionsHint` (v2.3.4) prevents double-mutation. Updated the claim-checker prompt with explicit per-tool coverage (which fields each mutation actually touches) so the LLM emits the flag reliably.

**Per-thread inbound queue (v2.5.0 A1).** New in `src/connectors/slack/inboundQueue.ts`. Three layered mechanisms wrapping every inbound Slack message: (1) **debounce** (1.5 sec) collapses rapid messages into one merged turn before Sonnet sees them; (2) **mutex** ensures only one orchestrator turn runs per `(channelId, threadTs)` at a time; (3) **abort-if-safe** — when a fresh message arrives during an in-flight turn AND no write tool has fired yet, abort the turn and restart with merged context. Once a write tool fires (`WRITE_TOOLS` set: create_meeting, message_colleague, coordinate_meeting, create_approval, all calendar mutations, all memory writes, etc.), abort is no longer safe — buffer for follow-up turn. `OrchestratorInput.signal?: AbortSignal` + `OrchestratorInput.onWriteExecuted?` callback wire the abort protocol; the tool dispatch loop fires `onWriteExecuted` BEFORE dispatching any write tool, sets the queue's `hasWriteFired` flag synchronously. Background callers (dispatchers, brief generation) omit both and run as before. Closes the parallel-orchestrator-runs root cause of the 2026-05-03 trace where one booking spawned 13+ tool calls.

**Per-turn cache via AsyncLocalStorage (v2.5.0 A3).** New in `src/utils/turnCache.ts`. `withTurnCache(fn)` wraps every orchestrator turn in an AsyncLocalStorage scope. Downstream code opts in via `memoize(key, fetch)` — first caller's promise is cached for the turn duration; concurrent callers within the same turn await the same promise instead of firing duplicate fetches. Currently used by `getCalendarEvents`; extend to other expensive reads as needed. Background callers see `getTurnCache()` return undefined and bypass memo. Cache lives only for the turn — GC'd automatically when the wrapped function returns. No cleanup code needed.

**Email-first booking schema (v2.5.0 C1).** `coordinate_meeting.participants` schema changed: `email` REQUIRED, `slack_id` OPTIONAL. Externals (no slack_id) auto-demote at the handler level into `just_invite` (calendar-invite-only path). Email is the booking primitive; Slack is bonus DM enrichment for internals. `find_slack_user` returns `{ external: true, email, message }` when query is an email outside owner's domain — explicit signal to proceed with email instead of treating empty result as a blocker. `find_slack_user` tool description rewritten to state it's for DM resolution only — NOT required before booking.

**Centralized `resolveSlackId` (v2.5.0).** `src/utils/resolveSlackId.ts`. Format check + people_memory fallback by name. Applied at message_colleague, update_person_profile, note_about_person, confirm_gender, log_interaction, create_approval. Silently recovers from Sonnet's slug hallucinations OR returns clean error pointing at find_slack_user.

**Owner-said-done scanner (v2.5.0).** `src/utils/closeLoopOnOwnerHandled.ts`. Fire-and-forget post-owner-turn pass. Cheap keyword pre-filter (EN+HE closure verbs) → if signals present + open items exist → single Sonnet pass classifies which items the owner said are done → cascades `cancel_task` / `cancel_coordination` / outreach `done`. Deterministic version of RULE 2d (which Sonnet drops half the time). ~30% of owner turns trigger the LLM; ~$0.001/turn average.

**`closeMeetingArtifacts` cascade extended (v2.5.0).** `calendar_dismissed_issues` now joins approvals/outreach/tasks as a 4th cascade target — every meeting mutation auto-resolves the related issue rows. Plus the v2.5.0 fix to the long-standing tasks-table column mismatch (was querying `payload_json` since v2.1.6; tasks table has `context` — silent SqliteError swallowed by try/catch). The third cascade target finally fires correctly.
