...
---
Architecture reference for Maelle **v2.6.1** (recent-outbound context for colleague DMs + exact-slot rule check w/ broken_rule_label + claim-checker specifics-mismatch shield bypass; v2.5.0 baseline = per-thread queue + per-turn cache + email-first booking schema; v2.4.x = floating-block + preferences catalog; v2.0.0 = Connection interface milestone).
