...
6. On approve with a replayable `on_approve.tool`, `runApproveCallback()` (`resolver.ts:640`) calls `runDeferredAction()` (`core/requests/deferredActionReplay.ts`) which re-invokes the exact original tool with an override flag (`relaxed: true` / `confirm_outside_window: true`) and stamps `_fulfilling_request_id` on the call so the booking-side cascade (`utils/closeMeetingArtifacts.ts`) knows to skip its own close+relay — this request already owns it.
