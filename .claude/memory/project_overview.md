...
**Current shipped version: 4.5.7.** `CHANGELOG.md` is the canonical, in-repo version-by-version history — it is not duplicated here or in `project_architecture.md`. Read it for what actually changed release to release; this file describes the durable shape, not the diff.
