...
- **E11 · THE COUNT IS THE FINDING — "or you get a patch party."** His words, and he called this **critical**. `node scripts/ledger-stats.cjs --index` prints how many times each promise has been broken. Seven breaks across four lanes is not seven bugs, it is **one missing mechanism, and the eighth site must not be patched** — patch it anyway and that's the patch party. **When the index flags a returning failure, the question you hand the lane CHANGES** — not *what is the root cause* but **why did the previous fix stop holding.** The old fix's proven line is right there, which makes a regression cheaper to diagnose than a fresh bug, not harder.
  ...
  **2026-08-16 · this is now wired at the `alreadyBuilt` door itself, not only your own reading of `--index`.** Any entry in the list `describeBuilt` shows you carries `regression: true`/`[REGRESSION]` when `--already-built --json` sees this exact identity closed AND open again. Drop a `[REGRESSION]` match in `droppedAsAlreadyBuilt` exactly as E12 says below — **the engine reinstates a clean `ref` match on its own**, reframed to "why did the fix not hold" with the prior `rootCause` carried forward, so you do not need to do anything differently for that case. The one case that still needs you: a match you can only make by **symptom** (no clean `ref` to key on) — the engine cannot reinstate what it cannot match, so emit that one as an ordinary issue yourself instead of a bare drop.
