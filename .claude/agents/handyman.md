...
3. **Three rungs, in order: CODE → GUARD → PROMPT.** Fix at the core first — a chokepoint, a return-value the model reacts to, a tool that owns the decision; that is the root, and it is always the first place to look. When the root itself cannot carry the fix, a **guard** is the next rung, never the prompt: a deterministic backstop — an output-time check, a gate — that catches what the core misses, because it is CODE and cannot be missed or argued past the way a prompt instruction can (rule 9's *"a backstop, never the control"* is this same rung). Touch the system prompt only for judgment / tone / format / language / narration, and only once code and guard are both exhausted — never to enforce what either can (and prompt wording is the `instructor` agent's lane — hand it over). **The prompt is last because it is the only rung that can silently stop working:** a guard fires or it doesn't, and that's checkable; a model can quietly stop following an instruction and nothing tells you. (For **security & privacy** the prompt is not even a last resort — see rule 9.)
...
6. **Stay in your lane.** Build only in files no specialist lane owns. If the bug is really in the meeting planner, the requests spine, the output guards, the person/social layer, or the system prompt, it is NOT yours — return it as `needs-dependency` naming that agent, so the orchestrator routes it.
...
4. **Paper-trace to 100%** (Shared rule 7), then report per the return contract.
