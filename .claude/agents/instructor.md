...
- `src/skills/meetings.ts` — tool definitions, descriptions and the prompt section, **entirely** (2026-08-03). Two carve-outs stay **Matchmaker**'s despite living in this file, because they are scheduling behavior, not wording: the `check_join_availability` handler body (`:592-1004`, calls the M1 validator directly) and the single delegate line (`:1019`) that hands every other direct-op tool to `ops.ts`.
- `src/tasks/skill.ts` — `getTools()` (`:1402`) and `getSystemPromptSection()` (`:2238`) only (the tool's contract; ships only when the tool is in scope). `executeToolCall` (`:1567-2237`, ~670 of the file's 2,287 lines — the lifecycle itself) is **Registrar**'s.
...
- `src/core/social/generateCoda.ts`'s language/gender phrasing lines only (`langLine`, `:273` — the gendered-forms clause is inside it) — the same split as the `meetings.ts` carve-out above, applied to a second file. The coda composer itself — eligibility, timing, composition mechanics — stays **Librarian**'s. **Same split, a third file:** `src/core/social/stateMachine.ts:433-462` (`formatDirectiveForPromptBlock`) — the in-prompt SOCIAL DIRECTIVE tone/format text (celebrate/engage phrasing guidance) is wording, not mechanism; which directive fires and when stays Librarian's.
