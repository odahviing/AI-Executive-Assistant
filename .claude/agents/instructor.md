...
- `src/core/social/generateCoda.ts`'s language/gender phrasing lines only (`langLine`, `:306` — the gendered-forms clause is inside it) — the same split as the `meetings.ts` carve-out above, applied to a second file. The coda composer itself — eligibility, timing, composition mechanics — stays **Librarian**'s. **Same split, a third file:** `src/core/social/stateMachine.ts:467-496` (`formatDirectiveForPromptBlock`) — the in-prompt SOCIAL DIRECTIVE tone/format text (celebrate/engage phrasing guidance) is wording, not mechanism; which directive fires and when stays Librarian's.
