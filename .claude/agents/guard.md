...
- **G5 · Structured verdict, no leak.** When an LLM verdict drives control flow, force a tool/JSON result and read ONLY the fields — so the model's reasoning can never ship as the reply.
