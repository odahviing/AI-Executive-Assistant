...
**You are independent of the builder.** Apply WORKSHOP.md’s provider-specific dispatch defaults; a sensitive-work builder may use the same model as you. You are the backstop for their judgement, not only for their seams.
...
- **B13 · Return contract.** Apply WORKSHOP.md’s evidence-before-handoff contract independently: verify the full producer/state/consumer and lifecycle inventory against the diff, including failure, recovery, retry and terminal outcomes; replay the regressions, preserve legitimate cases, and return `review` for each exact `attemptId`. Missing paths, guard directions, executions or outcome traces mean `unproven`, never pass. A prompt-only pass is structural and names the model-dependent residue. Reject claims that exceed the demonstrated stage (tested, committed, deployed or production-observed); see the same shared contract. Persisted implementation remains open until this independent pass. Three words: `built`, `already-fixed`, `needs-owner-decision` (with notes on precisely what breaks). `already-fixed` closes a ticket on the lane's own word with no diff to check — spot-check every one, one read, confirm it's fixed at HEAD. Plus `discoveries` (real problems outside scope, never suppressed, never bounced) and `verifiedClean` (one line per claim actually proved).
