...
Follow `.claude/WORKSHOP.md`'s **Who you are, and how to orient** section every dispatch — it is not restated here. **Your slice of `project_architecture.md`:** the "Transport layer" section — `connections/` (outbound) and `connectors/` (inbound), and where Slack's own pieces sit inside that split.

---

## What you own

**Inbound:** `src/connectors/slack/app.ts` + `app/` (the Bolt app, event routing, the inbound owner-clamp, reaction events, socket watchdog) · `inboundQueue.ts` (debounce + per-thread mutex + abort-if-safe) · `inboundReplayRegistry.ts` · `processedDedup.ts` · `socketWatermark.ts` · `recentOutboundContext.ts` · the Slack lookup/DM helper in `coordinator.ts` (`openDM` — `findSlackChannel` no longer exists, removed v4.2.x, zero callers; `findSlackUser` removed 2026-08-28, zero callers after the dead approval-dispatch block it fed was deleted) · `src/core/background.ts`'s catch-up apparatus (`catchUpMissedMessages`, `discoverThreadParents`, `findUnansweredTopLevel`/`findUnansweredInThread`/`findUnansweredMentionInThread`, `replayMissedMessage`) — dedup-and-catch-up after downtime (S8/S9), wired directly into `app.ts`'s socket watchdog and this lane's own dedup registries even though it lives outside `connectors/slack/`.
...
**Voice transcription:** the owner's approved budget is 180 seconds total for one invocation, shared by download/body, conversion and Whisper. Current `src/voice/index.ts` propagates cancellation to I/O, kills conversion and waits for child close before scratch cleanup; expiry does not fall back to a fresh transcription or paid retry. Handlers retain the existing failure notice and message-dedup lifetime; a new message can retry normally. This does not add durable delivery receipts or promise exactly-once audio delivery.
...
4. **Verify under W8 and the Workshop evidence contract** — cover all three surfaces (DM / MPIM / channel) plus the reconnect-and-catch-up path, which is where double-answers hide. Then report per the return contract.
