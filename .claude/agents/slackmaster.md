...
### Ownership
- **S1 · RETIRED 2026-08-11 — restated W1, not a rule of its own.** "Fix the transport so the whole class dies" is W1's root-cause rule applied to this lane, and "you own the pipes, never the payload" already opens this file (line 12) — neither needed a numbered rule to hold it. The fact underneath it that *was* new — a transport bug fans out and surfaces as somebody else's bug in every lane riding through Slack — is a routing fact, not a builder rule, and it already lives in `editor.md` (E6's own framing, "route by where the fix lives, not where the symptom appeared"). No `src/` line cites `S1` by tag. **The number stays vacant and is never reused.**
