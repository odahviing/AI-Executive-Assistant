...
- **A rough band, and it is a self-test, never a gate: under five rules is probably too small or too niche to be its own agent; over fifteen is probably one expertise that has become two.** Nobody is forbidden a sixteenth rule — a charter drifting outside the band is a prompt to look, not a violation. Measured 2026-08-01, and it holds without ever having been enforced: every builder sits between 8 and 14 (matchmaker 14 · profiler 13 · slackmaster 12 · **instructor 12** · diplomat 11 · registrar 10 · gatekeeper 9 · handyman 8), and the `framer` landed at 9. **Re-derive it, never copy it — `grep -cE '^- \*\*I[0-9]+' .claude/agents/instructor.md` per lane** (instructor went 11→12 on 2026-08-03 and this line was the stale citation). **The two zeros are not a verdict** — `bouncer` and `usher` hold real process expertise, written as prose and never numbered, which is why nothing can cite them by anything but a line number.
...
- **STRONG AT ITS POSITION, or merged. Every test above is about whether an agent should EXIST; this one is about whether it is doing the job at the seam it stands on, and it is the one that failed.** His ruling, 2026-08-03: *"Agents are supposed to be massive and meaningful. We don't build subagents just for fun. The bouncer was there and did a minimal job — that is not good, because the bouncer is the borderline to production. The usher needs to be strong to manage the team work. The cleaner needs to be strong to really clean Maelle. Agents doing small jobs should just merge with another agent."*

  **The bouncer PASSED every test above and was still weak.** It cleared the boot cost, held real process expertise, had a scope the usher could route on — and until 2026-08-03 it never asked *did this actually fix the reported problem*, never traced a multi-lane fix as one path, and had no observable on either, so a pass that did nothing and a pass that found nothing were the same artefact. Size at creation was never the question. **Depth at the position is.**
