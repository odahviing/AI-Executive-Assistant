...
**A new row takes the LOWEST FREE number, never the next one.** Merging duplicates frees ids and those gaps get filled, so the ledger stays small enough to hold in your head — his ceiling is about a hundred live rows, and he asked for the space to be reused rather than left behind. `architect-file.cjs` picks the number, so this is the minter's job and not something to remember. **A freed number is only free once every reference to its old occupant is gone** — the gate is a grep returning zero across the ledger, the report and this file, and until it does the row stays.
