...
- **G7 · An English pattern is an additive-only fallback, never a detector on its own.** It may add confidence to a detection something else already made; it can never fire alone (the ban on language-only detection as a primary or sole signal is W4's). Where the concern is purely semantic and no structural signal exists at all to gate on, this rule still wins the tie against G8's cheap-model path.
