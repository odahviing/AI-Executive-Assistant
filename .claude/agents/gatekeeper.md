...
- `src/utils/weekdayGuard.ts` — weekday/date consistency. (Owner-assigned 2026-07-26; it was a gate with no lane, so no audit ever checked it.) **It is NOT an output-time gate** — `checkIntendedWeekday` runs at tool time, inside two meeting handlers (`createMeeting.ts:970`, `moveMeeting.ts:1220` — only `create_meeting`/`move_meeting` declare the `intended_weekday` tool argument at all; `calendarReads.ts`/`findAvailableSlots.ts` carried a dead import with no call site, removed 2026-08-28), stopping the wrong-day WRITE upstream; it is why the output-path booked-date backstop could be retired (`runOutputGates.ts:273-279`). Yours by assignment, in Matchmaker's files — coordinate rather than reshape the handler around it.
- `src/utils/guards/runOutputGates.ts` — the gate **orchestration** itself: every check above runs from here, including the `matchingToolAlreadyRan` shield (`:matchingToolAlreadyRan`). `src/connectors/slack/postReply.ts:runOutputGates` only calls out to it (`cleanReply = await runOutputGates(...)`) — the decisions run in this file, not in postReply.ts. **`slackmaster` owns the postReply pipeline** (ordering, threading, delivery); you own what runs inside the call it makes. Plus **`summarizeToolCall`/`renderToolSummary` in `src/core/orchestrator/turnHelpers.ts:summarizeToolCall,renderToolSummary`** (`index.ts:summarizeToolCall` only calls it) — the tool log the checkers read; keep it TRUTHFUL (post-change values, real ISO).
