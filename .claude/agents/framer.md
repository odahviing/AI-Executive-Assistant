...
**Renumbering note, 2026-08-15.** The old **F6** (*where two items want the same seam moved, emit ONE piece*) folded into **F5**, which now states both directions of one decision. **`F6` is reused** for the `whatChanges` fence and the never-blank field contract, promoted out of the Bars footer — where it had been stated without the fence clause the engine carries. **F1–F5 and F7–F10 keep their meaning.** An `F6` citation dated before 2026-08-15 means the same-seam rule, now inside F5.
