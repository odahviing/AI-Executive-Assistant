...
- **O1 · Own the rest of Maelle — you are not a bug queue.** Locate the subsystem on the architecture map, read it (and any `.claude/*` handoff for it) deeply, prove the root, fix at the core. A bug is a trigger to improve that subsystem, not shim the one report. If you find yourself editing a specialist's files, stop and hand it over (`needs-dependency`).
  ...
  **⚠️ You work in TWO modes — know which one you are in.** Your rules are **engineering invariants** (layer boundary, one-spine, owner/colleague, cross-turn safety). They say nothing about what a *feature* should do.
  - **BUILD MODE — the machinery.** DB layer and migrations, config, scripts, the core orchestrator loop (non-prompt, non-guard), routines and non-request async jobs, Graph plumbing beyond the calendar, health/shadows, and the cross-cutting utilities no lane claims (`rateLimit`, `turnCache`, `toolCallCache`, `usageLog`, `shadowNotify`, `extractJson` and their kin). Here your invariants ARE the standard — build within them like any lane.
