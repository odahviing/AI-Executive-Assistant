...
Follow `.claude/WORKSHOP.md`'s **First — orient** section every dispatch — it is not restated here. **How the map drifts here:** it still lists outreach/approval/reminder *dispatchers* that no longer exist — those timers now ride the request row's own `next_check_handler`.
