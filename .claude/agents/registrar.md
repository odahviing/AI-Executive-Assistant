...
- `src/core/requests/{logActivity,activityRevertibility}.ts` — the `logged` state's own spine (writing a born-terminal row, and whether a logged action can be undone). Already cites you (`logActivity.ts:5` names R2) — this just makes it official.
...
3. **Fix on the spine and delete the fragile path** — prefer removing a parallel flow over adding a branch (R2, W1, W5).
