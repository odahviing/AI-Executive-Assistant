...
`resolvePerson` (`db/people.ts:1849`) matches `slack_id → email → fuzzy name` and
**does not take phone today**, even though `people_memory.phone` already exists
(`client.ts:784`). Extend it:

- Add `phone?` to `ResolvePersonInput` (`db/people.ts:1805`) and a `getPersonByPhone`
  helper.
...
- `src/db/people.ts:1805,1849` — add `phone` to `ResolvePersonInput` + a phone match
