...
`resolvePerson` (`db/people.ts:1408`) matches `slack_id → email → fuzzy name` and
**does not take phone today**, even though `people_memory.phone` already exists
(`client.ts:784`). Extend it:

- Add `phone?` to `ResolvePersonInput` (`db/people.ts:1364`) and a `getPersonByPhone`
  helper.
...
- `src/db/people.ts:1364,1408` — add `phone` to `ResolvePersonInput` + a phone match
