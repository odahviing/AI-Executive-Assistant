...
**Note on `architect`:** its "How you report back" section still carries its own verdict-first/no-preamble text rather than pointing here, and it states no shell-hygiene bar of its own — both untouched by the 2026-08-15 review, which only connected its A8 (cost) to this file's measure-never-estimate bar. Folding the rest in is a smaller follow-up, not urgent since A1 reserves every edit to that file for the owner regardless.
