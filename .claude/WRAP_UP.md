...
1. **Append EVERY row on `.claude/agent-loop/report.md` to `.claude/agent-loop/ledger.jsonl`** — whatever its verdict, not only the built ones. Two fields on every row, and both exist because this step dropped them:
   ...
   - **A `built` row this wrap actually ships gets a SECOND appended row, `"state":"wrapped"`, same `ref`, once the commit exists — never mutate the original line.** X152: measured 2026-08-04, 313 ledger rows still read `state:"built"` against only 18 `"wrapped"`, so this step has been skipped far more often than it has run — nothing durably distinguishes "in the tree, uncommitted" from "shipped weeks ago" without re-reading a `git log` for every ref by hand. (X152's own note guessed this also explains `alreadyBuilt` matching 0 of 94 refs one night — checked and **refuted**: `bugger.js`'s triage match is pure LLM judgment over the `ref`/`rootCause` text handed to it, with no code path that reads `state` at all, so a `built`-vs-`wrapped` gap cannot be the cause of that specific miss. Keep the two as separate defects.) Do this for every `built` ref this wrap's own commits carry, not only the ones that came in through `report.md` tonight.
...
**X152: whatever GitHub gets, the ledger gets the same statement, same step, never a second independent copy of the prose.** A closed-or-commented ticket with no matching ledger row is why *"a wave ships and the source GitHub issue is never closed or even updated, and nothing reports where a ticket stands"* took him a full day to notice himself. For every issue touched here, append one row to `.claude/agent-loop/ledger.jsonl`: `{"date":"<today>", "runId":"wrap-<version>", "ref":"gh#<n>", "verdict":"wrapped"|"needs-owner-decision", "state":"closed"|"partial", "note":"<the exact text gh issue close/comment sent>"}` — `state:"partial"` for a comment, `"closed"` for a close, and `note` is a copy of the actual GitHub text, not a paraphrase, so the two surfaces can never drift apart into two different ideas of what is done. This is the same act as step 9's `built`→`wrapped` append, run together: both read this wrap's `ticketCoverage` and its commit range once.
