...
- `src/utils/workingElsewhere.ts` + `src/utils/workHours.ts` — WE/away-day detection: `getEffectiveWorkDay`/`getEffectiveWorkDayForInstant` (yaml base ⊕ per-date `owner_schedule_overrides`, fail-safe to yaml) is now THE "where am I / in what zone" resolver on a day; `getTravelContextForInstant` is the thin adapter over it (#143 replaced the marker-based `resolveOwnerTravelContextForDate` + `weConfirmStash.ts` confirm-stash — away days rule-walk and book directly, no forced confirm).
