...
2 verify discoveries queued for the next build (not owed to you now): `requester-id-mint-tests-senderrole-not-authenticated-identity` (registrar, medium — same root class, one level upstream) and `createmeeting-owner-room-booking-self-requester-link` (matchmaker, low). `scanner-close-relay-targets-owner-self` shipped in `16dd175` (v4.8.0) — wrapped companion row appended, nothing left on this line to build.
