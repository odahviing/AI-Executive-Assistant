...
Your ruling, built: *"if a colleague want to move a meeting [or create] when someone else is busy, i don't care… just make sure yael knows."* One shared path now serves both doors: the availability walker tags EVERY blocked attendee instead of naming the first, and `attendeeConflictRefusal` in `violationLabels.ts` is the single refusal builder both handlers call, reusing the search path's own viewer-bound sentence rather than a paraphrase. The connector's own hand-typed copy of the reason codes was replaced by an import: one string pair, three readers. Owner-rule violations still escalate untouched, and your own bookings never enter this path.
