...
2. **Invoke the engine:** `Workflow({ name:'bugger', args:{ sources:['github','logs'], sinceIso:state.lastSeenIso, capBuilds:100, verify:true, priorClean:state.verifiedClean, alreadyBuilt:<symptoms of every BUILT row still uncommitted in report.md> } })`. Immediately record the returned `runId` into `state.lastRun` with `status:'running'` (so a mid-run lock is resumable).

   **`alreadyBuilt` is what makes unattended multi-day running work.** Builds are never deployed — they sit uncommitted until the owner wraps — so production keeps emitting the same symptom every night and the log review honestly re-finds it. The lane does catch it and returns `already-fixed`, but only after a full dispatch. Pass the symptom text of every built-but-uncommitted row and triage drops the repeat before it costs anything. **Nothing breaks without this; it just gets slower every night the owner is away.**
