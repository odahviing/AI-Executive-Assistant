...
- **The backlog is STORED in the ledger; its top reaches the report under the decision budget.** Every non-`built` row is open by definition, and `ledger-stats --open` is the whole list. **What he can act on tonight becomes rows in the report's one table** — bounded, ordered by harm, `confirmed` only; the rule and the bound are in the report spec below. A *copy* of the ledger in `report.md` is still a second source that drifts.
...
WORK
  ...
  backlog         the stale-row re-read on its own — it is already in every run
...
- **run** / **run now** — the full pass: open GitHub `Bug` issues + the log review since the watermark + **the stale-row re-read**, together. `sources:['github','logs','backlog']`, `sinceIso:state.lastSeenIso` — that is also the engine's default, so passing no `sources` gives all three. **This is the only way a run starts**; there is no timer.
- **backlog** — **already in every `run`, and this is that pass on its own** (`sources:['backlog']`). His ruling, 2026-07-30: *"put backlog in every run"* — new bugs and old bugs all need resolving, and the difference is bookkeeping. The scout runs `ledger-stats --open`, takes the rows flagged **RE-READ**, and returns `fixed` (naming the commit), `moved` (with the current `file:line`) or `still-real`. **A row is flagged for two reasons and the second is the large one (X59): its code MOVED, or NOBODY HAS EVER RE-READ IT** — 28 of 52 on 2026-07-30 against 0 that had moved, which is why the pass before it looked at one row. **The first pass is therefore the big one, and it is paid ONCE:** every re-read appends a `recheck` line, so that row is `confirmed` from then on and never returns to this list unless its code moves. It is not truncated silently either — `backlogSeen` against `backlogReread` is how a half-finished pass shows up as a number. **It builds nothing, and it does not spend his decision budget** — a re-read produces a closure or a confirmation. Take `persist.backlog`: append `closeInLedger` and delete those rows from `report.md`, **append `confirmInLedger` too** (that is what records "somebody looked", and without it the same rows are flagged again tomorrow), keep `keepInReport`. The rows `--open` lists under **`cite no file`** are the one part no pass can do: they are named at the end of that output and need HIS read.
...
2. **Invoke the engine:** `Workflow({ scriptPath:'.claude/workflows/bugger.js', args:{ sources:['github','logs','backlog'], sinceIso:state.lastSeenIso, capBuilds:100, verify:true, priorClean:state.verifiedClean, alreadyBuilt:<from the ledger>, openKnown:<from the ledger — see below> } })`. Immediately record the returned `runId` into `state.lastRun` with `status:'running'` (so a mid-run lock is resumable).
   ...
   **`openKnown` stops a SETTLED decision coming back as a fresh bug.** Two lists, two different facts. `alreadyBuilt` says *the fix exists, it just isn't deployed yet* — those stop recurring the moment he ships. `openKnown` says *this left the bug track*: `converted` into a GitHub issue where the design question is being worked. **Nothing is fixed in that second case, so the symptom recurs indefinitely** and an honest log review re-finds it every single night, forever.
   ...
   Derive it from the ledger as well: **every `converted` row, and nothing else** — passed as `[{ref, symptom, invariant, state, note}]`, with the GitHub ref in `note` so the scout can see where it went. **A `deferred` row NEVER goes on this list.** His ruling, 2026-07-30: *"defer for tomorrow or anything like this means defer to next run — the only thing we need is don't do it now."* A deferral is a **one-run skip**, so every `deferred` row standing when a run starts was deferred by an earlier run and is DUE now; passing one here tells the scout to drop work he asked for, which lost four rows on `wf_6852af85-afc`. If he never wants it at all, the verdict is `declined`. **Carry `invariant` on both lists and you have also handed the engine the slug vocabulary** — it harvests the distinct slugs out of them and names them in every lane dispatch, which is the only thing that stops three lanes calling one principle three names. Without this, rows 103 and 106 arrive tomorrow as brand-new bugs while their issues sit open, and he re-decides something he already decided.
...
1. **# · Lane · Status** — three facts, one cell, e.g. `104 · matchmaker · recommend build`.
   ...
   - **Status** — `built` (done, verified, wraps with everything else) · `pending owner — recommend <verb>` · `deferred` (he saw it and chose not to now — not the same as pending) · `declined` (he said no; stays visible so nobody re-raises it) · `in flight` (an agent has it now, mirrors `state.inFlight`) · `blocked (row N)`. **The verb is one of his five and there are no others: `build` · `decline` · `defer` · `resend` · `convert`.** **On `pending owner` the recommendation is not optional** — thirty unruled rows is thirty decisions, thirty with recommendations is **one decision plus exceptions** — and since X77 it is **checked**: `node scripts/ledger-stats.cjs --report` exits 1 naming any `pending owner` row whose Status cell has no `recommend` in it. `in flight` and `blocked` are the only exemptions, because those are owed to an agent rather than to him.
...
- **THE BACKLOG'S TOP IS IN THIS TABLE; THE BACKLOG ITSELF STAYS IN THE LEDGER.** His ruling, 2026-07-30: *"if we have bugs / issues, shouldnt we want to see if they are real and fix them? what we are getting from keeping them… its ladger, ladger is history. Im talking about the report, the place I take decision. Where is the misalignment?"* A ledger is history and a backlog is live work, and one file holds both — so live work sat where he does not decide. **The rows he can act on tonight are `pending owner` rows in this same table**, each with its recommendation (M6b), **sorted by M4 harm**, and **only rows `--open` reports as `confirmed`** — a row nobody has re-read is not a decision, it is homework, and `--open` now separates the two. **BOUNDED, AND THE BOUND GATES: this table's `pending owner` group never exceeds `capDecisions` (12).** The run's own rows come first; the backlog fills whatever room is left, **often none, and that is a correct answer.** Everything not carried is the headline's ledger total, which is where it already is. `node scripts/ledger-stats.cjs --report` prints the group against the budget. On 2026-07-26 thirty findings went into this file and he said the framework was not built to hold them — **the number was the problem, not the visibility.** A report showing everything open is a project tracker he scrolls past, which is worse than one showing nothing; a separate backlog section, or an `audit-backlog.md`, is still a duplicate that drifts.
