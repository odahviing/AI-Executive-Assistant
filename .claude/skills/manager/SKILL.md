...
- **backlog** — **already in every `run`, and this is that pass on its own** (`sources:['backlog']`). His ruling, 2026-07-30: *"put backlog in every run"* — new bugs and old bugs all need resolving, and the difference is bookkeeping. The scout runs `ledger-stats --open`, takes the rows flagged **RE-READ**, and returns `fixed` (naming the commit), `moved` (with the current `file:line`) or `still-real`. **A row is flagged for two reasons and the second is the large one (A59): its code MOVED, or NOBODY HAS EVER RE-READ IT** — 28 of 52 on 2026-07-30 against 0 that had moved, which is why the pass before it looked at one row. **The first pass is therefore the big one, and it is paid ONCE:** every re-read appends a `recheck` line, so that row is `confirmed` from then on and never returns to this list unless its code moves. It is not truncated silently either — `backlogSeen` against `backlogReread` is how a half-finished pass shows up as a number. **It builds nothing, and it does not spend his decision budget** — a re-read produces a closure or a confirmation. Take `persist.backlog`: append `closeInLedger` and delete those rows from `report.md`, **append `confirmInLedger` too** (that is what records "somebody looked", and without it the same rows are flagged again tomorrow), keep `keepInReport`. The rows `--open` lists under **`cite no file`** are the one part no pass can do: they are named at the end of that output and need HIS read.
- **report** — render `report.md` as the issue table (format below). **Cross-check it first, in BOTH directions.** (a) If `state.lastRun.notBuilt` is non-empty **or `ledger-stats --open` returns rows** and `report.md` shows nothing awaiting him, **the file is wrong** — say so plainly, name the ids, and rebuild it from the last run rather than reading out a file you know is false. This check exists because the report claimed "nothing is waiting" on a night four things were. (b) **Run `node scripts/ledger-stats.cjs --report`** — it compares every group heading and the headline against the rows beneath them and prints `MISMATCH`. Fix the file before you read it out; a count he cannot reconcile with the rows under it is what produced *"i'm not sure on the chapter of build, i just approve them all"*. A27: the head line said *"14 findings, 4 atomic, ZERO built"* and *"Seven rows await you"* above six rows of which four read `built`, and it stayed wrong for **four hours fifty-six minutes** across nine row-level edits that each flipped a status and never touched line 3. Direction (a) could not fire, because rows *were* awaiting him.
...
1. **# · Lane · Status** — three facts, one cell, e.g. `104 · matchmaker · recommend build`.
   - **#** — **a source letter plus the number from `state.nextReportId`: `G` github · `L` logs · `B` both · `O` owner** (he named it, or it came back off the report). Assign on first entry, increment, **never renumber and never reuse**, so `G176` means the same thing next month. **THIS IS THE ONLY PLACE THE SCHEME IS DEFINED.** His ruling, 2026-07-30, which reverses the flat-number rule that stood this morning: *"I still want one bug number… if you afraid about gh collison we can do preftix so G or GH is github, L is logs, O can be owner, I don't care but i need numbers."*
     **ONE counter, not one per letter** — the number alone is unique, so *"build 176"* resolves to exactly one row and the letter only says where it came from. That is what kills the collision he hit: `G176` can never be read as `gh#176`, and rows 164 and 165 coexisted with gh#164 and gh#165 while he rules by bare number. **There is no letter for `audit` or `verify`**, because neither is ever a row here — an audit gets its own session, and a discovery is the next build's intake. **New rows only:** existing ids keep their numbers and nothing is renumbered. A ledger slug (`category-classifier-blind-to-location`) is a stable identity for a BUG and belongs in `ref`; it is not a handle he can type, and putting one in this cell is what left `nextReportId` unused at 175.
   - **A derived row NAMES ITS PARENT as a suffix: `G156-a` for one complaint of a ticket, `G153-blockA` for a blocker raised on `G153`** (his design, 2026-07-30: *"if you had ticket 153 and you got something that block it, it won't be 170, it will be 153-blockA"*). **The letter and the suffix compose** — the letter says where the row came from, the number is the handle, the suffix says whose child it is. It buys two more things beyond the collision the letter already kills: the blocker states **what it blocks**, and coverage becomes **arithmetic** (a three-complaint ticket showing only `G156-a` is visibly incomplete with no coverage sentence written). **New rows only:** a parentless row is just letter-plus-number, existing ids keep what they have, nothing is renumbered.
