...
- **backlog** — **already in every `run`, and this is that pass on its own** (`sources:['backlog']`). His ruling, 2026-07-30: *"put backlog in every run"* — new bugs and old bugs all need resolving, and the difference is bookkeeping. It stays affordable because only the rows whose code MOVED are re-read, so the set is small; the scout runs `ledger-stats --open`, takes the rows flagged **RE-READ**, and returns `fixed` (naming the commit), `moved` (with the current `file:line`) or `still-real`. **It builds nothing, and it does not spend his decision budget** — a re-read produces a closure or a confirmation. Take `persist.backlog`: append `closeInLedger` and delete those rows from `report.md`, **append `confirmInLedger` too** (that is what records "somebody looked", and without it the same rows are flagged again tomorrow), keep `keepInReport`. The rows `--open` lists under **`cite no file`** are the one part no pass can do: they are named at the end of that output and need HIS read.
...
1. **# · Lane · Status** — three facts, one cell, e.g. `104 · matchmaker · recommend build`.
   ...
   - **A derived row NAMES ITS PARENT as a suffix: `156-a` for one complaint of a ticket, `153-blockA` for a blocker raised on row 153** (his design, 2026-07-30: *"if you had ticket 153 and you got something that block it, it won't be 170, it will be 153-blockA"*). A prefix hid the scheme; a suffix states it, and it buys three things: the blocker says **what it blocks**, coverage becomes **arithmetic** (a three-complaint ticket showing only `156-a` is visibly incomplete with no coverage sentence written), and the **GitHub collision goes** — rows 164 and 165 coexisted with gh#164 and gh#165 while he rules by bare number. **New rows only:** a parentless row keeps the flat counter, existing ids keep their numbers, nothing is renumbered.
...
**GROUP BY STATUS, then sort by harm inside each group** (M4: security → wrong action → silent wrongness → visible failure → polish), putting a root that explains several rows first. **`pending owner` · `deferred` · `built` · `declined`, each with its own count in the subheading** — his fix, 2026-07-30, because one undifferentiated list is how the open set stopped being visible. `in flight` and `blocked (row N)` sit in the `pending owner` group; they are owed, just not to him yet. **Grouping and sorting only — never a separate table per tier, and never a different shape per group.**
